...
\ea
\langinfo{Kɔnni}{}{\citealt[306]{Cahill2007}}\\
...
‘diarrhea’ ‘road’ ‘fear’ \\
\z\z
Grammatical tone, on the other hand, distinguishes one \textit{grammatical category} from another. There are many grammatical categories which can be thus distinguished. Some of the more common ones are given in \tabref{tab:PersonByGrammaticalTone:2} - \tabref{tab:OtherRelations:7} below.\todo{Please consider using example environments instead of tables for these} Not every person distinction is differentiated by tone in these or other languages; it is typically only two pronouns of the set that are so distinguished.
...
\label{sec:HowToneMarked:3}
  ...
  \label{sec:HowToneMarked:MarkGrammatical:3}
    Different languages have used a wide variety of strategies for indicating grammatical tone. One strategy is using diacritics, and often these mark a phonetic tone which instantiates a particular grammatical category, as in \tabref{tab:DiacriticPhoneticsMeaning:10}, with the Daffo variety of Lis Ma Ron.
...
Similarly, Etung (\tabref{tab:EtungPronouns}) uses 〈h〉 to differentiate pronouns which differ only by tone.
...
Other languages double some letters to differentiate pronouns which differ only by tone (\tabref{tab:JurModoLetterDoubling}).
...
A number of languages indicate various grammatical tone functions by means of punctuation or other non-alphabetic marks. Karaboro, as displayed in \tabref{tab:KaraboroPlurals}, uses a word-final hyphen to indicate plurals (in those cases which are not indicated by a segmental marker), which all happen to end in a low tone.
...
The old Ejagham orthography, now changed to a different system, used punctuation extensively to indicate various verbal aspectual forms (\tabref{tab:cahill:fiaghamOrtho:13}).
...
% %              (\citealt{Bird1999b}, corrected by John Watters pc)
    \begin{table}
        ...
        \caption{Old Ejagham orthography [etu] (Nigeria \& Cameroon)\\
             (\citealt{Bird1999b}, corrected by John Watters p.c.\ia{Watters, John})\label{tab:cahill:fiaghamOrtho:13}}
    \end{table}
...
Zinza [zin] (Echizinza) marks both lexical and grammatical tone, with accent marks for high, rising, and falling (see \tabref{tab:ZinzaAccentMarking}).
