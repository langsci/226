...
‘oil’ & {\textit{kpá\textsuperscript{!}}\textit{áŋ} }

‘guinea fowl’ & {\textit{kpàáŋ} }

‘back of head’ & \citealt[306]{Cahill2007}\\
{b. Mono [mnh]} 
...
‘fear’ & \citealt{Olson2005}: 198\\
\lspbottomrule
\end{tabularx}
\caption{Lexical tone differences in nouns}
...
a. Jur Modo  [bex]    \\
    (Sudan) & \textit{nì}       ‘her’ & \textit{ní}       ‘their’ & \citealt{Persson2004}: 80\\
{b. Lyele [lee]} 

    (Burkina Faso) & {\textit{ń}} 

2\textsc{sg} & {\textit{ǹ}} 

\textsc{3sg} & \citealt[57]{Kutsch2014}\\
\lspbottomrule
\end{tabularx}
...
\lsptoprule
a. Ndrulo [led]  \\
    (Uganda) & \textit{vìnì} & \textit{víní} & ‘his sister/s’ & \citealt[60]{Kutsch2014}\\
& \textit{djānì} & \textit{djání} & ‘his father/s’ & \\
b. Koro Waci [bqv]  \\
    (Nigeria) & \textit{ɪsʊr} & \textit{ɪsʊr} & ‘he-goat/s’ & Rachelle Wenger, pc\\
& \textit{ɪtɔmɪ} & \textit{ɪtɔmɪ} & {‘work/s’} & \\
& \textit{ìbǔr} & \textit{íbûr} & ‘slime/s’ & \\
c. Tarok  [yer] & \textit{ìfàng} & \textit{īfáng} & ‘fingers/s’ & \citealt{Longtau2008}: 90–91\\
     (Nigeria) &  &  &  & \\
...
\lspbottomrule
\end{tabularx}
\caption{Table: Singular/plural nouns distinguished by grammatical tone}
\label{tab:SingPluralGrammaticalTone:3}
\end{table}
...
\begin{table}
  ...
  (Nigeria) & \textit{ɔkɔn} & ‘you sang’ & \textit{móchí} & ‘he will eat’ & \citealt{Barnwell1969}\\
  ...
  \lspbottomrule
  \end{tabularx}
  \caption{Verbal aspect distinguished by grammatical tone}
  \label{tab:VerbAspectByGrammaticalTone:4}
\end{table}
...
\begin{table}
  ...
  (Sudan) & \textit{bàrù} & ‘country’ & \textit{bàrú} & ‘in the country’ & \citealt[61]{Kutsch2014}\\
            & \textit{dɔŋá} & {‘hand’} & \textit{dɔŋà} & ‘in the hand’ & \\
            & \textit{ʊtʊ} & ‘fire’ & ʊtʊ & ‘in the fire’ & \\
  \lspbottomrule
  \end{tabularx}
  \caption{Locative distinguished by grammatical tone}
  \label{tab:LocativeByGrammaticalTone:5}
\end{table}
...
\begin{table}
  ...
  \lsptoprule
  Sabaot [spy]\\
  (Uganda)  & \textit{kɪbakaac} \textit{kwààn} & ‘his father left him’ & \citealt[66]{Kutsch2014}\\
...
\begin{table}
  ...
  Lugungu [rub] & \textit{mulogo} \textit{muhandú} & ‘an old witch’ & \citealt[10]{Moe1999}\\
  (Uganda) & \textit{múlógó} \textit{muhandú} & ‘the witch is old’ & \\
            & \textit{múlógô} \textit{muhandú} & ‘the witch, (she) is \\
        old’ & \\
  \lspbottomrule
  \end{tabularx}
...
\label{sec:HowToneMarked:3}
  ...
  The consensus among linguists I have spoken to is that the common way of writing Hausa in \tabref{tab:VariousVerbalAspects:8} (there are other systems) is quite difficult to read. This is especially due to the fact that the \textit{grammatical} tone, as in the table, is not marked, and there are many situations where this ambiguity is impossible to resolve by the context.
    ...
    \begin{table}
      \begin{tabularx}{\textwidth}{XXXXX} & phonetic & orthography & gloss & \\
      \lsptoprule
       Hausa [hau] & [jáá tàfí] & ya tafi & ‘he went’ & \citealt{Harley2012}\\
       (Nigeria) & [jáà tàfí] & ya tafi & ‘he may go’ & \\
  ...
  \label{sec:HowToneMarked:MarkLexical:2}
    ...
    \begin{table}
      ...
      \lsptoprule
      extra high & high & mid & low & extra low & mid-low falling & low-high rising & high-low falling\\
      ″CV & ′CV & CV & {}-CV & =CV & CV- & {}-CV’ & ‘CV-\\
      \lspbottomrule
      \end{tabularx}
      \caption{Lexical tone notation for Côte d’Ivoire languages (Kutsch \citealt{Kutsch2014}: 58}
      \label{tab:LexicalToneNotationCoteLang:9}
    \end{table}
 ...
 %() Matthew 6:30a in Attié [ati]
  ...
  \label{sec:HowToneMarked:MarkGrammatical:3}
    ...
    \begin{table}
      \begin{tabularx}{\textwidth}{XXXXXX}
      \lsptoprule
      Lis Ma Ron [cla] & \textit{á} & \multicolumn{2}{X}{‘you (male)’} & \citealt{Harley2012}\\
      (Nigeria) & \textit{à} & ‘he’ & \multicolumn{2}{X}{}\\
      \lspbottomrule
      \end{tabularx}
...
Some languages indicate grammatical tone by letters which are otherwise unused. For example, Gangam [gng] (Togo) marks grammatical tone, not phonetically, but with other symbols to indicate the \textit{meaning}. The imperfective is marked with the letter <h> and the perfective with an apostrophe < ’ > (Higdon et al 2000, also \citealt{Roberts2013} for more examples. Phonetic transcription is from Jean Reimer pc.).

    \begin{exe}
        \ex \label{ex:ToneByUnusedLetters:3}
        \langinfo{Gangam}{}{Higdon et al 2000}
        \begin{xlist}
            \ex 
                N bɛnge' [bɛ́ŋge] Miganganm ya kaanm.\\
                'I learned to read Gangam.'\\
            \ex N laan bɛngeh [bɛ̄ŋgé] Miganganm ya kaanm nɛ.\\
...
A number of languages indicate various grammatical tone functions by means of punctuation or other non-alphabetic marks. Karaboro, for example, uses a word-final hyphen to indicate plurals (in those cases which are not indicated by a segmental marker), which all happen to end in a low tone.
    ...
    \begin{table}
        ...
        \[kāī, kāì\] & \textit{kai, kai-} & ‘affair, affairs’  \\
        \[gjɔɔ, gjɔɔ\]\& \textit{jɔɔ, jɔɔ-} & ‘net, nets’\\
        \[sààpjé, sàápjè\] & \textit{saapye, saapye-} & ‘rabbit, rabbits’\\
        \lspbottomrule
        \end{tabularx}
...
Table : Old Ejagham orthography [etu] (Nigeria \& Cameroon)\\
    ...
    \begin{table}
        \begin{tabularx}{\textwidth}{XXXX}
            ...
            apostrophe = \textsc{hortative} & [éme][F174?] & \textit{e’me} & ‘let us swallow’\\
...
The Bungu language is one of the more complex illustrations of grammatical tone marking. It uses both diacritics and punctuation marks to indicate the interaction of person and aspect in the verbal system. Many words are segmentally identical, and vowel length is not contrastive, putting a greater load on tone. At this point, no lexical tone is marked (though the orthography is still being adjusted), and 17 does not give the entire picture of grammatical tone. Other complexities exist as well, such as tone marking of objects. 
    ...
    \begin{table}
        ...
        orthography & gloss & \multicolumn{3}{X}{orthography} & \multicolumn{3}{X}{gloss}\\
        {\textit{w\"{a}kala}~}

        [wàkála] & ‘you bought (recent)’~ & \multicolumn{3}{X}{{\textit{:w\"{a}kala}}

        [wákala] } & \multicolumn{3}{X}{‘you have already bought’}\\
        {\textit{wákala}}

        [wákála] & ‘he bought (recent)’~ & \multicolumn{3}{X}{{\textit{:wákala}}

        [wakála] } & \multicolumn{3}{X}{‘he has already bought’}\\
        {\textit{waakala}}

        [waːkála] & ‘they bought (recent)’ & \multicolumn{3}{X}{{\textit{:waakala}}

        [wǎːkala] } & \multicolumn{3}{X}{‘they have already bought’~}\\
        {\textit{wakala}}

        [wakála]\textit{~}  & ‘they will buy’~~~~~~ & \multicolumn{3}{X}{{\textit{:nakala}}

        [nákala]} & \multicolumn{3}{X}{‘I have already bought’}\\
        {\textit{\^{}wakala~}}

        [wa\v{} kala]\textit{~~~}  & ‘they are buying’ & \multicolumn{3}{X}{{\textit{\^{}nakala} }

        [nǎkala] } & \multicolumn{3}{X}{‘I am buying’}\\
        Key: & \multicolumn{2}{X}{Colon = \textsc{completive}}  & \multicolumn{3}{X}{Carat = \textsc{progressive}} & \multicolumn{2}{X}{}\\
        & \multicolumn{2}{X}{Umlaut = \textsc{2sg.subj.past}} & \multicolumn{4}{X}{Accent = \textsc{3sg.subj.past}} & \\
        & \multicolumn{3}{X}{Double vowel = \textsc{3pl.subj.past}} & \multicolumn{4}{X}{}\\
        \lspbottomrule
        \end{tabularx}
        \caption{Bungu [wun] (Tanzania, Katterhenrich \& \citealt{Katterhenrich2016}) \textmd{(Low tone is unmarked)}}
...
\begin{table}
    \begin{tabularx}{\textwidth}{XXXs}
        \lsptoprule
         & deep & lexical and & \\
        Speech & orthography & phonemic orthographies & gloss\\
...
\begin{table}
    \begin{tabularx}{\textwidth}{XXXX}
    \lsptoprule
    speech & deep and lexical orthographies & phonemic orthography & gloss\\
    {[sɛ-tʊ]} 

    thanks-NC9 & <sɛtʋ> & <sɛtʋ> & ‘thanks’\\
    {[fɛyɪ]}

    there\_is\_no & <fɛyɩ> & <fɛyɩ> & ‘there is not’\\
    {[sɛ\textsuperscript{!}tʊ fɛyɪ]}

    thanks-NC9 there\_is\_no & <sɛtʋ fɛyɩ> & <sɛ’tʋ fɛyɩ> & ‘don’t mention it!’\\
    \lspbottomrule
    \end{tabularx}
...
\begin{table}
    ...
    orthography & lexical L tone spreading & post-lexical HLH plateauing\\
    Phonemic & {Written as pronounced}

    (easier) & {Written as pronounced}

    (harder)\\
    {Lexical} &  & {Written without post-lexical processes}
