...
Examples~to illustrate some speech sound errors 

\begin{tabularx}{\textwidth}{XXX}
\lsptoprule
\textbf{French} \textbf{word}  & \textbf{illustration} & \textbf{Error} \textbf{types}\\
«~viande~»  & [vjãd] → [bjãd] & Substitution \\
«~couscous~»  & [kuskus] → [tutu] & substitution; omission\\
«~chaise»  & [ʃɛz] → [ʃɛ] & omission\\
«~table~»  & [tabl] → [tab] & omission\\
«~chocolat~»  & [ʃokola] → [kokola] & consonant harmony\\
\lspbottomrule
\end{tabularx}
...
\lsptoprule
\textbf{School} \textbf{level} & \textit{N} & \textbf{\%} \\
\textit{Maternelle} (kindergarten) & \textit{136}  & 12\\
\textit{SIL} (grade 1) & \textit{518}  & 46 \\
\textit{CP} (grade 2) & \textit{473}  & 42 \\
\textbf{Gender}  &  & \\
\textit{girls} & \textit{553}  & 49 \\
\textit{boys} & \textit{574}  & 51 \\
\textbf{Sociolinguistic} \textbf{context}  &  & \\
\textit{rural} & \textit{606}  & 54 \\
\textit{urban} & \textit{521}  & 46\\
\textbf{\textit{Total} } & \textit{1127}  & \textit{100} \\
\lspbottomrule
\end{tabularx}
...
\begin{tabularx}{\textwidth}{msX}
\lsptoprule
\textbf{Structure}  & \textbf{Error}  & \textbf{Phonological} \textbf{process} \textbf{leading} \textbf{to} \textbf{the…}\\
...
\begin{tabularx}{\textwidth}{XXXXXXXXX} & \multicolumn{2}{c}{ \bfseries Gender}  & \multicolumn{2}{c}{ \bfseries Sociolinguistic context} & \multicolumn{3}{c}{ \bfseries School level} & \bfseries Total \\
\lsptoprule
& Girls & Boys & Rural & Urban & Mat & SIL & \textit{CP} & \\
\hline
\bfseries N & 553 & 574 & 606 & 521 & 136 & 518 & 473 & \textbf{1127}\\
\bfseries n & 25 & 43 & 39 & 29 & 12 & 39 & 17 & \textbf{68}\\
\bfseries \textit{\%} & \textit{4.5} & \textit{7.5} & \textit{6.4} & \textit{5.6} & \textit{8.8} & \textit{7.5} & \textit{3.6} & \textbf{\textit{6.0}}\\
\lspbottomrule
\end{tabularx}
\label{tab:takam:3}
\end{table}
...
\lsptoprule
& {\bfseries 4 years} \bfseries (N=117) & \bfseries 5 years (N=254) & \bfseries 6 years (N=279) & \bfseries 7 years (N=221) & \bfseries 8 years (N=256)\\
\hline
n & 8 & 16 & 19 & 11 & 14\\
{\%} & 6.8 & 6.3 & 6.8 & 5.0 & 5.4\\
\lspbottomrule
...
\lsptoprule
\bfseries Age groups & \bfseries Not impaired & \bfseries Low & \bfseries Medium & \bfseries Highly \\
\hline
4 years (n=8) & [p, m, n, ɲ, ɥ ] & n/a & [j, v, d, w, f, t, b, k] & [ʒ, g, s, z, ʃ, l, ʁ]\\
...
\bfseries 4-8 years (n=68) & \textbf{[p,} \textbf{m,} \textbf{n]} & \textbf{[ɲ,} \textbf{v,} \textbf{f,} \textbf{ɥ,} \textbf{ʒ,} \textbf{j,} \textbf{w,} \textbf{b,} \textbf{t,} \textbf{d]} & \textbf{[g,} \textbf{k]} & \textbf{[s,} \textbf{ʃ,} \textbf{z,} \textbf{l,} \textbf{ʁ]}\\
\lspbottomrule
\end{tabularx}
\end{table}
...
\lsptoprule
& \bfseries 4 years & \bfseries 5 years & \bfseries 6 years & \bfseries 7 years & \bfseries 8 years & \bfseries 4-8 years\\
\hline
 & n=8 & n=8 & n=8 & n=8 & n=8 & n=8\\
\bfseries CV & \textit{6.2} & \textit{31.2} & \textit{36.8} & \textit{27.3} & \textit{28.6} & \textit{32.3}\\
\bfseries VC & \textit{75.0} & \textit{37.5} & \textit{57.9} & \textit{27.3} & \textit{35.7} & \textit{52.9}\\
\bfseries \MakeUppercase{CC} & \textit{75.0} & \textit{68.7} & \textit{31.6} & \textit{36.4} & \textit{35.7} & \textit{52.9}\\
\bfseries CVC & \textit{87.5} & \textit{93.7} & \textit{94.7} & \textit{90.9} & \textit{100} & \textit{97.0}\\
\lspbottomrule
\end{tabularx}
\label{tab:takam:6}
\end{table}
...
\lsptoprule
& \multicolumn{2}{c}{ \bfseries \textit{4-8} \textit{years}} & \bfseries \textit{Chi-square} ($\chi$\textsuperscript{2})\\
\hline
\textbf{Errors} & n & \textit{\%} & \\
{o}mission & 68 & \textit{100} & $\chi$\textsuperscript{2}=(4, N=68)= 5.26, \textit{p}=.26\\
{S}ubstitution & 61 & \textit{89.7} & $\chi$\textsuperscript{2}=(4, N=68)= 4.36, \textit{p}=.36\\
{D}istortion & 43 & \textit{63.2} & $\chi$\textsuperscript{2}=(4, N=68)= 7.12, \textit{p}=.10\\
Addition & 32 & \textit{44.1} & $\chi$\textsuperscript{2}=(4, N=68)= 5.69, \textit{p}=.22\\
Assimilation & 22 & \textit{32.4} & $\chi$\textsuperscript{2}=(4, N=68)= 4,28, \textit{p}=.37\\
\lspbottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
\item \textit{syllable final consonant omission}\\
(ardoise) [aʁ-dwaz] > [a-dwaz] $\rightarrow$ V\textbf{C} > V\\
(couscous) [kuskus] > [ku-kus] $\rightarrow$ CV\textbf{C} > CV\\

\item \textit{syllable initial consonant omission}\\
(parapluie) [pa-\textbf{ʁ}a-plɥi] > [pa:-plɥi] $\rightarrow$ CV>V\textbf{:}\\
(arachide) /a-\textbf{ʁ}a-ʃid/ >  /a:-ʃid/ $\rightarrow$ CV>V\textbf{:}\\

\item \textit{simplification of consonants group}\\
(crayon) [kʁe-jɔ] > [ki-jɔ] $\rightarrow$ C\textbf{C}V > CV\\
(table)  [tabl] > [tab] $\rightarrow$ CVC\textbf{C} > CVC\\

\item \textit{syllable omission}\\
(nourriture) [nu-ʁ\textbf{i}-tyʁ] > [nu:-ty:] $\rightarrow$ CV\\
(télévision) [te-le-\textbf{vi}-zjɔ] > [te-le-sjɔ] $\rightarrow$ CV\\

\item \textit{simplification of complex syllable structures}\\ 
(arbre)  [a:ʁbʁ] > [a:b] $\rightarrow$ V\textbf{C}C\textbf{C} > VC\\
(parapluie) [pa-ʁa-plɥi] > [pa-ha-ply] $\rightarrow$ CC\textbf{C}V > CCV\\
\end{itemize}
\end{exe}
...
\item \textit{substitution} \textit{of} \textit{[l]} \textit{to} \textit{[j]}\\
...
\item \textit{substitution of [ʃ,ʒ] to [s,z]:}\\ 
...
\item \textit{substitution of [l] to [ʁ]}\\
...
\item \textit{substitution of vowel to glide}\\
(parapluie) [paʁaplɥi] > [palaply] $\rightarrow$ [ɥ]> [y]\\

\item \textit{Devoicing}:\\
...
\textbf{Special} \textbf{thanks}: to Dr. John Ogwana for verifying the transcribed data and for all his contributions in this work, to Pr. Geneviève Bengono who performed the medical exam, and to Claire Nkoué, the speech-language therapist who gave us access to the speech and language assessment material used during the survey. Our special thanks also go to the teachers and headmasters of the eight schools for their enthusiasm and willingness to make the survey possible.
