...
\ex\label{comp} \begin{xlist}
		\ex[]{\gll Ekpe\ix{i} a-bo ke	(\textbf{im\d{o}})\ix{i} \textbf{i}-ma \textbf{i}-to Udo\\
		...
		\ex[]{\gll Ekpe\ix{i} a-bo ke i-ma i-t\d{o} \textbf{idem}\ix{i} \\
...
\begin{exe}
	\ex{\gll Ol\'u\ix{i} ti k\'ede NO\ix{\set{i}[+LOG]} p\'e oun\ix{i}/\textbf{o}\ix{i/k} \`n b\s{\`o} l\s{\'o}la\\
	...
	\ex{\gll yapi\ix{i} hE kO \textbf{O}\ix{k}/n\ix{i,(k)} ye sE\\
...
\begin{exe}
	\ex{\gll \d{o}mm\d{o}\ix{i} e-ke e-bo ke \textbf{mmim\d{o}}\ix{i/*k} i-ma i-kot \={n}wet\\
...
\begin{exe}
	\ex{\gll Ekpe\ix{i} a-bo ke \textbf{mmim\d{o}}\ix{\set{i,k}} i-diya af\d{i}t adesi ad\d{o}\\
	...
	\ex{\gll Ekpe\ix{i} a-bo ke Udo\ix{j} a-kere ke ete \textbf{mmim\d{o}}ix{\set{i,j}/\set{j,k}} a-ya i-di i-w\d{o}\\
...
\begin{context}
		...
		\glt `Ekpe\ix{i} said that he\ix{i/k} sings well.' }
			\ex[\#]{\gll Ekpe\ix{i} a-bo ke \textbf{im\d{o}}\ix{i} i-me i-diy\d{o}\={n}\d{o} ikwo ikwo mf\d{o}nmf\d{o}n\\
...
\begin{exe}
	...
	\ex[*]{\gll Ekpe\ix{i} a-ma a-kop ke Udo\ix{k} a-ke a-bo ke \textbf{im\d{o}}\ix{i/k} i-ma i-t\d{o} \textbf{im\d{o}}\ix{i/k} \\
...
\begin{exe}
	...
	\gll Olu\ix{i} so p\'e Ade\ix{k} ro p\'e b\`ab\'a \textbf{oun}\ix{i/k} ti r\textipa{\`\bari} \textipa{\`bari}y\'a \textbf{\`oun}\ix{i/k}\\
	...
	\gll Kofi\ix{i} xɔ-e se be Ama\ix{k} gblɔ be \textbf{y\`e}\ix{i/k}-ʄu \textbf{y\`e}\ix{i/k}\\
			Kofi receive-PRO hear that Ama say that \textsc{log}-beat \textsc{log}\\
...
\begin{exe}
	...
	\gll vɨzeri Rojda Bill-ra va kɛ \textipa{\textbf{3z}} \textbf{to}-ra miradi\u{s}a\\
...
\begin{exe}
	\ex{\gll Ad\'e\ix{i} so p\'e \textbf{oun\ix{i}} ti r\textipa{\`\bari} \textipa{\`\bari}w\'e \textbf{r\`e\ix{i,j}}\\
			Ade say that oun \textsc{perf} see book o-gen \\
			\glt `Ade\ix{i} said that he\ix{i} has seen his\ix{i,j} book.}\label{control}
	\ex{\gll Olu\ix{i} so p\'e \textbf{o\ix{\**i/j}} r\textipa{\`\bari} b\`ab\'a \textbf{\`oun\ix{i}}\\
			Olu say that o see father oun-gen\\
			\glt `Olu\ix{i} said that he\ix{\**i/j} has seen his\ix{i} father.'}\label{blocked}
	\ex{\gll Olu\ix{i} so p\'e b\`ab\'a \textbf{r\`e\ix{i/j}} r\textipa{\`\bari} \textipa{\`\bari}y\'a \textbf{\`oun\ix{i}}\\
...
\end{context}
\begin{exe}
	\ex{\gll Heseni va kɜ \textbf{Rojdaa} layik \textbf{tɨya} pach kerd\\
			Hesen\textsc{.obl} said that Rojda.\textsc{obl} boy your kiss did\\ \hfill \small \citep[][333]{Anand2006}
...
\begin{exe}
	\ex{\gll Ekpe\ix{i} a-ma a-bo ke \textbf{im\d{o}\ix{i}} i-ma i-k\d{i}t ete \textbf{am\d{o}\ix{i/k}}\\
