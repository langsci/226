...
\ea  Somali nominals \label{ex:downing:1}
  ...
  \begin{tabbing}
  béer  ~ \textup{‘liversfdg’} \= vs.sdf \= beér ~ \textup{‘garden’}\kill
  \textit{ínan} ~ \textup{‘boy’} \> vs.  \> inán   ~ \textup{‘girl’}\\
  \textit{béer}  ~ \textup{‘liver’} \> vs. \> beér ~ \textup{‘garden’}\\
  \textit{éy} ~ \textup{‘dog’}  \>  vs.  \> eý ~ \textup{‘dogs’}\\
    \end{tabbing}
...
compound:   (keli (tális)\textsubscript{PWord-max})\textsubscript{CWG}  (* keli dális)\\
