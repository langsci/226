...
\begin{figure}
\caption{Event structures for actions \citep[33]{Botne2010}\todo[inline]{improve}}
...
\matrix[matrix of nodes,nodes in empty cells,nodes={align=center}] (matrix) {
 ...
 \path[draw,rounded corners=2pt] (matrix-1-1.north east) to (matrix-1-1.north west) to (matrix-1-1.south west) to (matrix-1-1.south east);
 \path[draw,rounded corners=2pt,dotted] (matrix-1-3.north west) to (matrix-1-3.north east) to (matrix-1-3.south east) to (matrix-1-3.south west);
 \draw (matrix-1-3.north west) -- (matrix-1-3.north east);
 \draw (matrix-1-3.south west) -- (matrix-1-3.south east);
