...
\title{The interaction of ‑ø‑\ldots‑íle  with aspectual classes in Nyamwezi}
...
According to \citeauthor{Guthrie1967}'s \citeyear{Guthrie1967} widely acknowledged classification system, Nyamwezi is classified as F22. There are four dialects of Nyamwezi according to \citet{Masele2001}. These include Kinyanyembe, Kidakama, Sigalaganza and Kikonongo. The description in this paper is based on the Kidakama dialect, spoken in Uyui district.
...
% % % \todo{redo all diagrams with tikz}
\begin{figure}
...
\begin{figure}
\caption{Event structures for actions \citep[33]{Botne2010}\todo[inline]{Please check and approve}}
...
\matrix[matrix of nodes,nodes in empty cells,nodes={align=center}] (matrix) {
 ...
 \node[fit=(matrix-3-1) (matrix-3-2)] {Activity};
...
\matrix[matrix of nodes,nodes in empty cells,nodes={align=center}] (matrix) {
    & &[1cm] & & &[1cm] & \\
    O & N & O & N & C & N & C\\
    & \vphantom{fj}&[1cm] & & &[1cm] &\vphantom{fj} \\
};
\node[fit=(matrix-3-1) (matrix-3-2)]{Inceptive};
\node[fit=(matrix-3-3) (matrix-3-5)]{Transitional};
\node[fit=(matrix-3-6) (matrix-3-7)]{Resultative};
...
\todo{check all floats whether they are tables or figures}
\begin{figure}
...
\glll waaβɪtaga apeel\textbf{ilé}, a\textbf{l}\textbf{ɪɪ}ja kaaya \\
    ...
    3\textsc{sg}-\textsc{cpl}-pass-\textsc{rec}-\textsc{fv} 3\textsc{sg}-run-\textbf{íle}  3\textsc{sg}-\textbf{PROG}-go-\textsc{fv} homestead \\
...
\glll waaβɪtaga a\textbf{l}\textbf{ɪɪ}peela, az\textbf{íílé}         kaáya\\
    ...
    3\textsc{sg}-\textsc{cpl}-pass-\textsc{rec}-\textsc{fv} 3\textsc{sg}-\textbf{PROG}-run-\textsc{fv} 3\textsc{sg}-go-\textbf{íle} homestead\\
...
\glll *waaβɪtaga a\textbf{l}\textbf{ɪɪ}peela,  a\textbf{l}\textbf{ɪɪ}ja kaaya\\
      ...
      3\textsc{sg}-\textsc{cpl}-pass-\textsc{rec}-\textsc{fv} 3\textsc{sg}-\textbf{PROG}-run-\textsc{fv} 3\textsc{sg}-\textbf{PROG}-go-\textsc{fv}  homestead \\
