...
\glossify{3SG-\textbf{CPL}}-finish-\textbf{ile}\\
...
cl.\textsc{6}-begin-\glossify{CPL}-start-\glossify{REC-FV} \glossify{INF}-cool.off-\glossify{FV} but cl.\glossify{6-NEG}-cool.off-íle good\\
...
\ea 
    ...
    ka-\textbf{lɪɪ}-saat-a\\                              
     cl.12-\textbf{\glossify{PROG}}-be.sick\\            
...
\begin{figure}
\todo[inline]{Compare with original}
\begin{tikzpicture}
...
\begin{tikzpicture}
\matrix[baseline,nodes={anchor=base,draw,minimum width=2cm,minimum height=1.5cm,text height=2ex,text depth=1ex}] {
\node (O) {(O)}; & \node [minimum width=4cm] (N) {N}; & \node (C) {(C)};\\
};
\draw[dashed,thick,white] (O.north west) -- (O.south west);
\draw[dashed,very thick,white] (C.north east) -- (C.south east);
\end{tikzpicture}
\end{figure} 
...
\begin{tikzpicture}
\matrix[baseline,nodes={anchor=base,draw,minimum width=4cm,minimum height=1.5cm,text height=2ex,text depth=1ex}] {
\node (O) {(O)}; & \node (N) {N};\\
};
\draw[dashed,thick,white] (O.north west) -- (O.south west);
\draw[dashed,very thick,white] (N.north east) -- (N.south east);
\end{tikzpicture}
\end{figure}
